...
# --- Configuration ---
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "AIzaSyBjSxcIkD6E3bZMvw2hHArNcQanhpxcwIE")
FIREBASE_SERVICE_ACCOUNT_PATH = os.environ.get("FIREBASE_SERVICE_ACCOUNT_PATH", "firebase_service_account.json")
...
def decode_google_news_url_advanced(encoded_url):
    ...
    try:
        ...
        if '/articles/' in encoded_url:
            ...
            # Try hex decoding
            try:
                if len(article_id) % 2 == 0:
                    decoded_bytes = bytes.fromhex(article_id)
                    decoded_str = decoded_bytes.decode('utf-8', errors='ignore')
                    url_pattern = r'https?://[^\s<>"\']+[^\s<>"\'.,;!?]'
                    urls = re.findall(url_pattern, decoded_str)
                    for url in urls:
                        if 'google.com' not in url and len(url) > 20:
                            logger.info(f"✅ Hex decoded URL: {url}")
                            return url
            except Exception as e:
...
async def fetch_real_news_with_newsapi(keywords):
    ...
    """
    try:
        newsapi_key = "4ec906af6ade46939ec74607fca25db5"  # Get from newsapi.org
    ...
    try:
        ...
        for script in json_scripts:
            try:
                ...
                image_url = extract_image_from_structured_data(data)
                if image_url and is_real_news_image(image_url):
                    absolute_url = urljoin(response.url, image_url)
                    logger.info(f"✅ Found JSON-LD image: {absolute_url}")
                    return absolute_url
            except:
                continue
        
        # Priority 4: Article images with enhanced selectors
        article_selectors = [
            'article img[src]',
        ...
        for selector in article_selectors:
            ...
            for img in images[:3]:  # Check first 3 matches
                src = img.get('src')
                if src and is_real_news_image(src):
                    absolute_url = urljoin(response.url, src)
                    logger.info(f"✅ Found article image: {absolute_url}")
                    return absolute_url
...
def extract_image_from_structured_data(data):
    ...
    if isinstance(data, dict):
        ...
        for value in data.values():
            if isinstance(value, (dict, list)):
                result = extract_image_from_structured_data(value)
                if result:
                    return result
    
    elif isinstance(data, list):
        for item in data:
            result = extract_image_from_structured_data(item)
            if result:
                return result
    ...
    try:
        ...
        for search_term in search_terms:
            try:
                ...
                pexels_api_key = "YOUR_PEXELS_API_KEY"  # Replace with actual key from pexels.com/api
...
async def fetch_real_news_newsdata(keywords):
    ...
    try:
        ...
        api_key = "pub_30c7c17f4c4840f7bb7fd0c016688e11"
...
@app.route('/api/test/resolve/<path:url>')
async def test_url_resolution(url):
    """Test enhanced URL resolution"""
    try:
        decoded_url = decode_google_news_url_advanced(url)
        session_resolved = await resolve_google_news_with_session(url)
        ...
        return jsonify({
            ...
            "sessionResolvedUrl": session_resolved,
            "isGoogleNews": 'news.google.com' in (session_resolved or decoded_url or url),
            "success": bool(decoded_url or session_resolved)
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/api/test/image/<path:url>')
async def test_image_extraction(url):
    """Test enhanced image extraction"""
    try:
        image_url = await extract_real_image_aggressive(url, "Test Article")
        return jsonify({
            "url": url,
            "extractedImage": image_url,
            "success": bool(image_url)
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/api/test/pexels/<query>')
async def test_pexels_search(query):
    """Test Pexels API search"""
    try:
        image_url = await get_relevant_image_from_pexels(query, "")
        return jsonify({
            "query": query,
            "pexelsImage": image_url,
            "success": bool(image_url)
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500
